...
function displayMovies(movies) {
    ...
    for (const movie of movies) {
        ...
        <div class="flex flex-col items-center justify-around gap-2 rounded text-white p-2 bg-[#2a1825] border border-[#362431] h-full w-full lg:w-1/10">
    ...
    async function bookmarkedTVshow() {
        ...
        await fetch(`${window.location.origin}/cinetech/isLogged`)
            ...
            .then(data => {
                if (data) {
                fetch(`${window.location.origin}/cinetech/getBookmarksMovies`)
            .then(response => response.json())
            .then(data => {
                console.log(data);
                if (data.error) {
        ...
        for (const movie of movies) {
            ...
            if (btnAddToWatchlist){
                btnAddToWatchlist.addEventListener('click', async (event) => {
                    event.preventDefault();
                    await fetch(`${window.location.origin}/cinetech/addBookmarks/${movie.id}/movie`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                const containerBtnBookmark = btnAddToWatchlist.parentElement;
                                containerBtnBookmark.innerHTML = `
                                    <button type="button" id="btnRemoveFromWatchlist" class="flex items-center gap-2" data-id="${movie.id}">
                    ...
                    await fetch(`${window.location.origin}/cinetech/removeBookmarks/${movie.id}`)
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                displayMessageToast(containerModalDialog,'Série retirée de votre watchlist', 'error');
                                bookmarkedTVshow();
                            }
                        });
                });
